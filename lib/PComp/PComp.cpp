...
PObject::Status_t Drive::initialize(void)
{
    ...
    PID_Init(&settings_.m1Pid, settings_.m1PidKp, settings_.m1PidKi, settings_.m1PidKd, settings_.m1PidMin, settings_.m1PidMax);
    PID_Init(&settings_.m2Pid, settings_.m2PidKp, settings_.m2PidKi, settings_.m2PidKd, settings_.m2PidMin, settings_.m2PidMax);
...
PObject::Status_t Drive::run(void)
{
    ...
    int iterations = 0;
    const int MAX_ITERATIONS = 100000;  // Safety limit to prevent infinite loop

    while (t1 + 10 > PObject::getTickMs() && iterations < MAX_ITERATIONS)
    {
        iterations++;
        m1Curr = settings_.m1TachGpio->read();
        if (m1Curr >= 0 && m1Curr != m1Prev) { m1Count++; m1Prev = m1Curr; }

        m2Curr = settings_.m2TachGpio->read();
        if (m2Curr >= 0 && m2Curr != m2Prev) { m2Count++; m2Prev = m2Curr; }
    }
...
{
    settings_.m1DirGpio->shutdown();
    settings_.m2DirGpio->shutdown();

    settings_.m1TachGpio->shutdown();
    settings_.m2TachGpio->shutdown();
